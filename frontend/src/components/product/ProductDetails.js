...
const ProductDetails = ({match}) => {
  ...
  const decreaseQte = ( )=> {
    ...
    if (count.valueAsNumber <= 1) return;   //quantity in stock 
    const qty = count.valueAsNumber - 1;
    setQuantity(qty)
  }
        ...
        { loading ? <Loader/> :(
         ...
         <MetaData title={product.name} />
            ...
            <div className="col-12 col-lg-5 mt-5">
              ...
              <span id="no_of_reviews">({product.numOfReviewrs} Reviews)</span>
      
              <hr/>
      
              <p id="product_price">${product.price}</p>
              <div className="stockCounter d-inline">
                  <span className="btn btn-danger minus" onClick={decreaseQte}>-</span>
      
                  <input type="number" className="form-control count d-inline" value={quantity} readOnly />
      
                  <span className="btn btn-primary plus" onClick={increaseQte}>+</span>
              </div>
